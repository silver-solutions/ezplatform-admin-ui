(function(global, doc, eZ, Translator) {
    ...
    const initField = (field) => {
        const fieldValue = JSON.parse(field.dataset.value);
    ...
    const addVariant = (field, values) => {
        ...
        itemsElements.forEach((itemElements) => {
            ...
            itemInputs.forEach((item) => {
                item.addEventListener('change', () => saveData(field), false);
    ...
    const attachEvents = (field) => {
        field.querySelector('.ez-btn--add-variant').addEventListener('click', () => addVariant(field), false);
    };
    const saveData = (field) => {
        ...
        itemsGroups.forEach((itemsGroup) => {
            ...
            const itemData = {
                id: field.querySelector('.ez-custom-dropdown__select').value,
                sku: {
                    label: skuLabel,
